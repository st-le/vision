...
def keypointrcnn_vis_loss(keypoint_logits, keypoint_vis_logits, proposals, gt_keypoints, keypoint_matched_idxs):
    # type: (Tensor, List[Tensor], List[Tensor], List[Tensor]) -> Tensor
    # copied and modified from keypointrcnn_loss
    ...
    valid_gt = torch.cat(valid, dim=0).to(dtype=torch.uint8)
    valid = torch.where(valid_gt)[0]

    # torch.mean (in binary_cross_entropy_with_logits) does'nt
    # accept empty tensors, so handle it sepaartely
    if keypoint_targets.numel() == 0 or len(valid) == 0:
        return keypoint_logits.sum() * 0, 0

    keypoint_vis_loss = F.binary_cross_entropy_with_logits(
            keypoint_vis_logits.view(-1), valid_gt.to(torch.float32)
    )

    keypoint_logits = keypoint_logits.view(N * K, H * W)

    keypoint_loss = F.cross_entropy(keypoint_logits[valid], keypoint_targets[valid])
    return keypoint_loss, keypoint_vis_loss
...
def keypointrcnn_vis_inference(x, xv, boxes):
    # type: (Tensor, List[Tensor]) -> Tuple[List[Tensor], List[Tensor]]
    kp_probs = []
    kp_scores = []
    kp_vis = []
    ...
    boxes_per_image = [box.size(0) for box in boxes]
    x2 = x.split(boxes_per_image, dim=0)
    xv2 = xv.split(boxes_per_image, dim=0)

    for xx, xxv, bb in zip(x2, xv2, boxes):
        kp_prob, scores = heatmaps_to_keypoints(xx, bb)
        kp_probs.append(kp_prob)
        kp_scores.append(scores)
        kp_vis.append(xxv)

    return kp_probs, kp_scores, kp_vis
...
class RoIHeads(nn.Module):
    ...
    def forward(
        ...
        if (
            ...
            keypoint_vis_logits = self.keypoint_vis_predictor(keypoint_features)
            ...
            if self.training:
                ...
                rcnn_loss_keypoint, rcnn_loss_vis_keypoint = keypointrcnn_vis_loss(
                    keypoint_logits, keypoint_vis_logits, keypoint_proposals,
                    gt_keypoints, pos_matched_idxs)
                loss_keypoint = {
                    "loss_keypoint": rcnn_loss_keypoint,
                    "loss_vis_keypoint": rcnn_loss_vis_keypoint
                }
            else:
                ...
                keypoints_probs, kp_scores, kp_vis = keypointrcnn_vis_inference(keypoint_logits, keypoint_vis_logits, keypoint_proposals)
                for keypoint_prob, kps, kpv, r in zip(keypoints_probs, kp_scores, kp_vis, result):
